...
Группа `quality`. Применяется в `04-prototype/` и `05-code/` для анимаций,
микровзаимодействий и polish.

Не заменяет:
- `quality/design-review` — соответствие дизайн-системе
- `quality/design-decompose` — свет, типографика, кромки
- `quality/design-anti-slop` — ИИ-паттерны

Часто вызывается из `build/ds-to-storybook` и `build/flow-to-app`.
