...
### 3a. Дизайнер (координатор UX/UI)
**Функция:** Собрать продуктовый дизайн после ТЗ, не подменяя аналитика
- Маршрутизирует работу по метасреде `design/` (TemplateDesign)
- Делегирует субагентам research / content / ds / prototype
- Публикует `{artifacts_dir}/design_spec.md` для архитектора и планировщика
- **НЕ** переписывает ТЗ и **НЕ** пишет продуктовый код

### 3b. Ревьюер дизайна
**Функция:** Проверка design_spec и публичных outputs дизайн-сред
- Соответствие ТЗ, трассировка, полнота состояний UI, безопасность артефактов
- **НЕ** правит дизайн сам
...
### Этап 1a: Дизайн (если есть UI/UX)
```
ТЗ → Дизайнер → субагенты design/* → design_spec
                    ↓
              Ревьюер дизайна → Замечания
                    ↓
              Дизайнер (доработка)
```
**Циклы:** Максимум 2 итерации review

**Пропуск:** нет пользовательского интерфейса — сразу архитектура

### Этап 2: Проектирование архитектуры
```
ТЗ + design_spec + Описание проекта → Архитектор → Архитектура
                            ↓
...
- Передавать между агентами только файлы текущего шага (минимальный контекст)
